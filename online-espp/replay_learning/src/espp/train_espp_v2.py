...
def main() -> None:
    ...
    path = "./bin/bptt/espp/per_layer_training/two_layer/sum_echo_100_epochs/"
    fig_path = path + "plots/"
    ...
    espp_layer_trainer_1 = ESPPLayerTrainer(
        espp_layer=espp_layer_func(in_channels=2, out_channels=12),
        lr=lr,
    ...
    espp_layer_trainer_2 = ESPPLayerTrainer(
        espp_layer=espp_layer_func(in_channels=12, out_channels=32),
        lr=lr,
        train_metrics=multi_metric_builder.set_tag("train").build(),
        val_metrics=multi_metric_builder.set_tag("val").build(),
        test_metrics=multi_metric_builder.set_tag("test").build(),
    ...
    espp_trainer.train(EPOCHS, layer_in_pbar=1)

    print("Few Shot Training...")
    layers = [espp_layer_trainer_1.espp_layer.layer, espp_layer_trainer_2.espp_layer.layer]
    train_loss, test_loss, train_acc, test_acc = train_few_shot(layers, 32*5*5, train_loader, test_loader, device)
