...
def main() -> None:
    ...
    # path = "./bin/bptt/mi_full_online_ff/imu_fortiss/B1_ws0.4s_f100Hz_binary_input_thr_no_log_gamma1.0_gamma1.0_temp05_temp025_lr8e-2_lr4e-3_100_epochs_3_few_shot_ripple_replay20_shuffle/"
    path = "./bin/tdcl_full_online/imu/ff/B1_bins"+str(bins)+"_beta"+str(beta)+"_"+str(tracer)+"_warmp_up"+str(warm_up)+"_inp_thr"+str(input_thr)+"_lr(8e-2_4e-3)_tmp(0.5_0.25)_gamma1.0_epochs"+str(EPOCHS)+"/"
    # path = "./bin/tdcl_full_online/imu/ff/B1_bins40_beta0.5_(-0.871399998664856_0.9)_(1.1815999746322632_0.85)_carry_warmp_up20_inp_thr0.0_lr(8e-2_4e-3)_tmp(0.5_0.25)_gamma1.0_epochs100"
    if path[-1] != "/":
        path += "/" 
    fig_path = path + "plots/"
    ...
    train_loader, val_loader, test_loader = create_imu_dataloaders(batch_size=BATCH_SIZE, window_length=bins, stride=bins, num_workers=4, drop_last=True, binary=True, balanced_sampling=False, split_data=False)
    ...
    tdcl_layer_trainer_1 = FullOnlineTDCLLayerTrainer(
        tdcl_layer=tdcl_layer_func(in_features=60, out_features=512, lr=8e-2, gamma=1.0, temp=0.5, tracer=tracer.clone()),
